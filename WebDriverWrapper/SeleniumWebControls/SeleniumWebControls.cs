...
namespace WebDriverWrapper
{
    public class SeleniumWebControls : IControl
    {
        ...
        { 
            get
            {
                if (aWebElement.Text.Equals("") || aWebElement.Text == null)
                {
                    WebDriverWait wait = new WebDriverWait(aControlAccess.Browser.BrowserHandle, TimeSpan.FromMinutes(1));
                    wait.Until(x => x.FindElement(Utility.GetByFromLocator(aControlAccess.LocatorType, aControlAccess.Locator)).Text.Length > 1);
        ...
        public bool WaitForEleVisible()
        {
            
            try
            {
                WebDriverWait wait = new WebDriverWait(aControlAccess.Browser.BrowserHandle, TimeSpan.FromMilliseconds(30000));
                wait.Until(ExpectedConditions.ElementIsVisible(Utility.GetByFromLocator(aControlAccess.LocatorType, aControlAccess.Locator)));
                return true;
            }
            catch (Exception)
        ...
        public bool WaitForFrameVisible()
        {
        ...
        private void WaitForClickable()
        {
            ...
            wait.Until(ExpectedConditions.ElementToBeClickable(Utility.GetByFromLocator(aControlAccess.LocatorType, aControlAccess.Locator)));
        }
